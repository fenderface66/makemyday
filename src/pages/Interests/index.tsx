        ...
        >
          <Formik
            ...
            validate={(values: Values) => {
              ...
              if (values.interest_scenes.length < 8) {
                errors.interest_scenes =
                  "You must select at least 8 interest images";
              }
              return errors;
            }}
            onSubmit={async (values, { setSubmitting, resetForm }) => {
              const interests = flatten(
                values.interest_scenes.map((scene) => interestSceneMap[scene])
              );
          ...
          >
            {({ isSubmitting, values }: FormikProps<Values>) => (
              <Form>
                <h2>
                  Please select the images that best represent your interests
                </h2>
                <MasonryContainer role="group" aria-labelledby="checkbox-group">
                  ...
                  >
                    {images.map((image) => (
                      <label
                        key={`label-${image.name}`}
                        data-testid={`image_${image.name}`}
                      >
                        <SelectableImage
                          key={image.name}
                          image={image}
                          checked={values.interest_scenes.includes(image.name)}
                        />
                      </label>
                    ))}
                  </Masonry>
